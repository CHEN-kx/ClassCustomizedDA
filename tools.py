...
def image_classification_test_semi(loader, base_net, classifier_layer, test_10crop, config, num_iter):
    start_test = True
    with torch.no_grad():
        if test_10crop:
            ...
            for i in range(len(loader['test'][0])):
                ...
                for j in range(10):
                    ...
                    output = classifier_layer(features_base.to(dtype=classifier_layer.weight.dtype))
    ...
    predict = predict+shared_class_num
    class_recall = {}
    class_accuracy = {}
    ...
    return accuracy * 100.0, 0.0, 0.0, 0.0, 0.0
...
def image_classification_test_clip(loader, model, text, test_10crop, config, num_iter):
    start_test = True
    with torch.no_grad():
        if test_10crop:
            iter_test = [iter(loader['test'][i]) for i in range(10)]
            for i in range(len(loader['test'][0])):
                ...
                outputs = []
                for j in range(10):
                    logits_per_image, logits_per_text = model(inputs[j], text)
...
def image_classification_test_clipvis(loader, base_net, classifier_layer, test_10crop, config, num_iter):
    start_test = True
    with torch.no_grad():
        if test_10crop:
            iter_test = [iter(loader['test'][i]) for i in range(10)]
            for i in range(len(loader['test'][0])):
                ...
                outputs = []
                for j in range(10):
                    features_base = base_net.encode_image(inputs[j]).to(dtype=classifier_layer.weight.dtype)
